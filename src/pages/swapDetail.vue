<template>
  <div class="vault_wrap">
    ...
    <div class="vault_inner_wrap">
      ...
      <div class="pools-main" v-if="swapRcd.records.length > 0">
        <div class="pools__item" v-for="(item,index) in swapRcd.records" :key="index">
                ...
                <ul class="pools__rows">
                  <li class="pools__row-1">
                    <div class="pools__logo-name">
                      ...
                      <div class="pools__coin-name">{{coin1(item)}}</div>
                    </div>
                    <svg-icon icon-class="enter_icon"></svg-icon>
                    <div class="pools__logo-name">
                      <svg-icon class="pools__coin-logo" :icon-class="coin2(item) + '_coin'" />
                      <div class="pools__coin-name">{{coin2(item)}}</div>
                    </div>
                  </li>
                  <li class="pools__row pools__apy">
                    <div class="pools__labe-field">{{$t('l.t_transamount')}}</div>
                    <div class="pools__label-value pools__label-value--black">{{amt1(item) + ' / ' + amt2(item)}}</div>
                  </li>
                  <li class="pools__row">
...
import Swapr from '@/models/swapr';
...
export default {
  ...
  methods: {
    ...
    coin1(item){
      let index = item.currencyIndex
      
      let name = getCurrencyName(index)
      console.log({name})
      return name == 'USDT' ? 'LBR' : 'USDT'
    },
    coin2(item){
      let index = item.currencyIndex
      let name = getCurrencyName(index)
      return (name == 'Libra' ? 'LBR' : name) || 'LBR'
    },
    amt1(item){
      let amount = item.usdtAmount
      return  (amount / Wallet.Precisions()).toFixed(4)
    },
    amt2(item){
      let amount = item.libAmount
      return  (amount / Wallet.Precisions()).toFixed(4)
    },
    formatTimeStr(item){
  ...
  async mounted() {
    // this.queryRecords()
    this.swapRcd.address = localStorage.getItem("walletAddress");
    this.swapRcd.querySize()
  },
  destroyed() {
  }
}
