  ...
  <div class="vault_wrap">
    ...
    <div class="vault_inner_wrap">
      ...
      <div class="vault_seg_nav">
        <ul class="vault_left">
          <li v-for="(item,index) in navArr" :key="index" @click="handleContChange(index)" class="valut_nav_item" :class="currentIndex == index ? 'active' : ''">{{item}}</li>
                ...
                <ul class="pools__rows">
                  <li class="pools__row-1">
                    <div class="pools__logo-name">
                      ...
                      <div class="pools__coin-name">LBR</div>
                    </div>
                    <div class="pools__logo-name">
                      <div class="pools__coin-name pools__apy-value">+{{amt(item)}}</div>
                    </div>
                  </li>
                  <li class="pools__row" v-if="item.incomeType">
                    <div class="pools__labe-field">{{$t('l.t_profittype')}}</div>
                    <div class="pools__label-value">{{formatTypeStr(item)}}</div>
                  </li>
                  <li class="pools__row">
...
export default {
  ...
  data() {
    return {
      ...
      records:[],
      dataSize:0,
      navArr: [this.$t('l.t_profitD'),this.$t('l.t_wde_a')],
      currentIndex:0,
      totalProfit:0,
      takeOutProfit:0,
      profit:new Profits(),
      profittypes:[
        {type:1,desc:this.$t('l.t_holdfen')},
        {type:2,desc:this.$t('l.t_shangfan')},
        {type:3,desc:this.$t('l.t_ganen')},
        {type:4,desc:this.$t('l.t_quanqiu')},
      ]
    }
  },
  computed: {
  ...
  methods: {
    ...
    async handleContChange(index) {
      ...
      try {
          if(this.currentIndex == 0){
            ...
            if(count > 0){
              this.dataSize = count
              await this.profit.getProfitRecord(0,this.dataSize)
            }
          }else{
            ...
            if(count > 0){
              this.dataSize = count
              await this.profit.getTiquRecord(0,this.dataSize)
            }
          }
    ...
    formatTypeStr(item){
      ...
      let fixed = this.profittypes.find(ele => ele.type == type)
      return fixed.desc || '-'
    },
    formatTimeStr(item){
