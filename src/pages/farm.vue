<template>
  <div class="vault_wrap">
    ...
    <div class="vault_inner_wrap">
      <div class="vault_top_nav">
        <!-- <ul class="vault_nav shadow">
          <li v-for="item in navArr" :key="item.index" @click="handleContChange(item.index)" class="valut_nav_item" :class="currentIndex == item.index ? 'active' : ''">{{$t('l.'+item.name)}}</li>
          <div id="donghuaBox" :style="{left:(currentIndex == 1? 0 : 1)*50+'%'}"></div>
        </ul>
        <div class="copy_wrap">
              ...
              <a-spin tips="loading" :spinning="item.isLoading" size="large">
                <ul class="pools__rows">
                  ...
                  <li class="pools__row">
                    ...
                    <div class="pools__label-value pools__label-value--black"><countTo :endVal='$formatDecimal(item.lockAmount,4)' :duration='3000' :decimals="4"></countTo></div>
                  </li>
                  <li class="pools__row">
                    <div class="pools__labe-field">{{$t('l.VL')}}({{item.currency}})</div>
                    <div class="pools__label-value"><countTo :endVal='$formatDecimal(item.totalLockAmount,4)' :duration='3000' :decimals="4"></countTo></div>
                  </li>
                  <li class="pools__group-buttons">
                    <div class="pools__button-group">
                      ...
                      <a-button v-show="!item.isApproved || !item.isApprovedExtra" :loading="item.isApproving" class="g-button pools__dialog__option g-button-heco-theme " @click="handleApprovedFor(item.currency,item)">{{item.isApproving ? $t('l.t_approving') : $t('l.approve')}}</a-button>
                      <button v-show="item.isApproved && item.isApprovedExtra"  class="g-button pools__dialog__option g-button--approved" @click="handleShowDepositModal(0,item)"><svg-icon icon-class="Star_icon_white"></svg-icon> <span>{{$t('l.deposit')}}</span></button>
                    </div>
                  </li>
                </ul>
              ...
              <ul class="pools__rows">
                <li class="pools__row-1">
                  <div class="pools__logo-name">
                    ...
                    <div class="pools__coin-name" :class="currentIndex == 1 ? 'name_lp_2' : ''">{{item.currencyC1}}/{{item.currency2}}</div>
                  </div>
                  <div class="pools__info">{{$t('l.reward')}} LBR</div>
                </li>
                <li class="pools__row pools__apy">
                  <div class="pools__labe-field">{{$t('l.nhsyl')}}</div>
                  <div class="pools__apy-value"><countTo :endVal='item.rateOfAnnualized' :duration='3000' :decimals="4" suffix="%"></countTo></div>
                ...
                <li class="pools__row">
                  <div class="pools__labe-field">{{$t('l.deposited')}}({{item.currencyC1}}/{{item.currency2}})</div>
                  <div class="pools__label-value pools__label-value--black">
                    <countTo :endVal='$formatDecimal(item.lockAmount1,4)' :duration='3000' :decimals="4"></countTo>
                    /
                    <countTo :endVal='$formatDecimal(item.lockAmount2,4)' :duration='3000' :decimals="4"></countTo>
                    </div>
                </li>
                <li class="pools__row">
                  <div class="pools__labe-field">{{$t('l.VL')}}({{item.currencyC1}}/{{item.currency2}})</div>
                  <div class="pools__label-value">
                    <countTo :endVal='$formatDecimal(item.totalLockAmount1,4)' :duration='3000' :decimals="4"></countTo>
                    /
                    <countTo :endVal='$formatDecimal(item.totalLockAmount2,4)' :duration='3000' :decimals="4"></countTo>
                    </div>
                </li>
                <li class="pools__group-buttons">
                  <div class="pools__button-group">
    ...
    <in-drawer ref="inner" :info="depositInfo" @sure="mobileDepositClick" @changeApprove="updateCoinApprove"></in-drawer>
...
export default {
  ...
  data() {
    return {
      ...
      depositInfo:{},
      approveInfo:{},
      currentIndex: 1,   //0 单币  1 lp
      navArr: [{name:'lp',index:1},{name:'db',index:0}],
      iptValue: undefined,
      incomeAmount: 0,
      iptValue1: undefined,
      iptValue2: undefined,
  ...
  computed: {
    ...mapGetters('accounts',{walletAddress:'getActiveAccount',getDataUpdateTime:'getDataUpdateTime',isMain:'getIsMainChainID',getIsMobile:'getIsMobile'}),
  },
  methods: {
    handleTapStart(e) {
    ...
    updateCoinApprove(coins){
      let _self = this
      //1. farms approvedTokens 状态

      //2. farms ones,twos 授权状态

      coins.forEach((ele) => {
        _self.farms.approveTokens = _self.farms.approveTokens.map(info => {
          const dict = {...info}
          if(info.currency == ele){ dict.isApproved = false }
          return dict
        })
        _self.farms.ones = _self.farms.ones.map(info => {
          const dict = {...info}
          if(info.currency == ele){ dict.isApproved = false }
          return dict
        })
        _self.farms.twos = _self.farms.twos.map(info => {
          const dict = {...info}
          if(info.currency1 == ele){ dict.isApproved1 = false }
          if(info.currency2 == ele){ dict.isApproved2 = false }
          return dict
        })
      })
    },
    ...
    async handleApprovedFor(currency1,item) {
      let _self = this
      _self.farms.address = _self.walletAddress
      _self.farms.upperAddress = _self.inviteAddress
      _self.farms.currency1 = currency1
      _self.farms.currency2 = item?.currencyExtra || item?.currency2
    ...
    async handleWithDrawOne(currency,amount) {
      ...
      _this.farms.address = _this.walletAddress
      _this.farms.upperAddress = _this.inviteAddress
    ...
    async handleShowDepositModal(index,item) {
      ...
      // 这里传入 gas 信息
      let currencyInfo_1 = _self.farms.approveTokens.find(ele => {return (ele.currency == item.currency || ele.currency == item.currency1)})
      let currencyInfo_2 = _self.farms.approveTokens.find(ele => {return ele.currency == item.currency2})

      _self.depositInfo.gas1 = currencyInfo_1?.gas || 0
      _self.depositInfo.gas2 = currencyInfo_2?.gas || 0
      ...
      _self.depositInfo.balance = 0;_self.depositInfo.balance2 = 0;_self.depositInfo.mPrice = 0; _self.depositInfo.sPrice = 0;
      ...
      if(res.length > 3) _self.depositInfo.balance2 = res[3];
    ...
    async handleDepositConfirmTwo(libraAmount,currency2) {
      ...
      _this.farms.currency2 = currency2
      const two = _this.farms.depositTwoCurrency()
      two.then(res => {
        if(res) {
            _this.isModalShowSaveTwo = false
            _this.$refs.inner.close()
            _this.$refs.loading.success({title:_this.$t('l.l_success')})
            setTimeout(() => { _this.farms.refreshData() },3000)
        }
      })
      .catch(err => {
        let msg = JSON.stringify(err.message || err)
  ...
  async mounted() {
    let _self = this
    //从首页过来的
    let ptype = this.$route.query.ptype ? this.$route.query.ptype : 2
    if(ptype !== undefined) {
    ...
    setTimeout(async () => {
       ...
       .then((res) => {
        ...
        if(!_self.isMain){ //检查网络
          _self.$message.warning(_self.$t('l.notmainnet_tips'));
          _self.farms.ones.forEach(ele => ele.isLoading = false)
          _self.farms.twos.forEach(ele => ele.isLoading = false)
          return;
        }
        _self.checkUserInfo() //同步上下级信息
