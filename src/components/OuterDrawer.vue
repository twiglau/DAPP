    ...
    >
      <div class="container">
          ...
          <div class="container__coin">
              ...
              <span>{{model.isSingle == 1? `${$transformCurrency(model.currency1)}/${model.currency2}` : `${model.currency}`}}</span>
          </div>
          <div class="container__desc">
              <span>{{$t('l.deposited')}}({{$transformCurrency(model.currency || model.currency1)}})</span><span><countTo :endVal='$formatDecimal( model.isSingle == 0? model.lockAmount : model.lockAmount1,4)' :duration='1000' :decimals="4"></countTo></span>
          </div>
          <div class="container__input">
              <input @input="input_num(1)" :placeholder="$t('l.iptPlace')" v-model="iptValue1">
              <div class="input__info">
                  <span>{{model.isSingle == 1? `${$transformCurrency(model.currency1)}/${model.currency2}` : `${model.currency}`}}</span>
                  <a-button class="max-btn" @click="iptValue1 = $formatDecimal(model.isSingle == 0? model.lockAmount : model.lockAmount1,4)">{{$t('l.l_maxa')}}</a-button>
              </div>
          </div>
          ...
          <div class="container__info" style="margin-top:8px">
              <div class="info-l">
                  <span>{{$transformCurrency(model.currency || model.currency1)}}</span>{{$t('l.withdrawal')}}
              </div>
              <div class="info-r">
          ...
          <div class="container__info">
              ...
              <div class="info-r">
                  <div class="info-fee">
                     ...
                     <span>{{model.isSingle == 0? model.currency : $transformCurrency(model.currency1)}}</span>
                  </div>
          ...
          <div class="container__ratio">
              <span>{{$t('l.l_you')}}</span>
              <span>
                {{model.isSingle == 0? `${model.nResult} ${model.currency}` : `${model.nResult1} ${$transformCurrency(model.currency1)} + ${model.nResult2} ${model.currency2}`}}
                </span>
          </div>
...
export default {
  ...
  methods: {
    ...
    sureClick(){
        let amount = parseFloat(this.iptValue1)
        this.$emit('sure',amount)
    },
  }
}
</script>
