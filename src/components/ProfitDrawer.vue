    ...
    >
      <div class="container">
          ...
          <div class="container__coin">
              ...
              <span>{{`${$transformCurrency(model.currency)}`}}</span>
          </div>
          <div class="container__desc">
              <span>{{$t('l.l_unprofit')}}({{$transformCurrency(model.currency)}})</span><span><countTo :endVal='$formatDecimal(model.lockAmount,4)' :duration='1000' :decimals="4"></countTo></span>
          </div>
          <div class="container__input">
              <input @input="input_num(1)" :placeholder="$t('l.iptPlace')" v-model="iptValue1">
              <div class="input__info">
                  <span>{{`${$transformCurrency(model.currency)}`}}</span>
                  <a-button class="max-btn" @click="iptValue1 = $formatDecimal(model.lockAmount,4)">{{$t('l.l_maxa')}}</a-button>
              </div>
          </div>
