...
const state = {
  ...
  mainChainID: '0x38', //主链ID,用于判断是否处于主链上, 16进制, 0x61（97） 为测试链    0x38（56）为主网
  isMainChainID: true,
  isActiveUpper:false,
};

const getters = {
  getIsActiveUpper(state){
    return state.isActiveUpper
  },
  getIsMobile(state){
...
const mutations = {
  ...
  async disconnectWallet(state) {
    ...
    state.isActiveUpper = false;
    state.upperAccount = null;
  ...
  setIsActiveUpper(state,isActiveUpper) {
    state.isActiveUpper = isActiveUpper;
  },
