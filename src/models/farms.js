...
import {error} from '@/utils/errorLog.js';
...
function Farms(address,upperAddress){
    ...
    //授权状态
    this.approveTokens = [
      {currency:"ETH",isApproved:false,gas:0},
      {currency:"Diem",isApproved:true,gas:0},
      {currency:"BNB",isApproved:true,gas:0},
      {currency:"BTC",isApproved:false,gas:0},
      {currency:"USDT",isApproved:false,gas:0},
      {currency:"Libra",isApproved:false,gas:0},
      {currency:"FIL",isApproved:false,gas:0},
      
      {currency:"ADA",isApproved:false,gas:0},
      {currency:"XRP",isApproved:false,gas:0},
      {currency:"DOGE",isApproved:false,gas:0},
      {currency:"DOT",isApproved:false,gas:0},
      {currency:"SOL",isApproved:false,gas:0},
      {currency:"UNI",isApproved:false,gas:0},
      {currency:"BCH",isApproved:false,gas:0},
      {currency:"LTC",isApproved:false,gas:0},
      {currency:"LINK",isApproved:false,gas:0},
      {currency:"EOS",isApproved:false,gas:0}
    ];
    //某个币种授权信息
    this.approveInfo = {};
}
...
Farms.prototype.getMySingleCoinsLockData = async function(){
  ...
  if(one && one > 0){
    ...
    .catch(err => _self.ones.forEach(ele => ele.isLoading = false))
  }else{
    _self.ones.forEach(ele => ele.isLoading = false)
  }
...
Farms.prototype.getMyPairCoinsLockData = async function(){
  ...
  if(two && two > 0){
    ...
    .catch(err => {
      _self.twos.forEach(ele => ele.isLoading = false)
    })
  }else {
    _self.twos.forEach(ele => ele.isLoading = false)
  }
  ...
  new Promise(async (resolve,reject) => {
    try {
        for(let i = 0; i < _self.ones.length;i++){
          let in_out = await new Promise((res,rej) => {
          ...
          _self.ones[i].totalLockAmount = in_out
        }
        //全部请求
        resolve('success')
    }catch(err){
...
Farms.prototype.updateCurrencyApprovedStatus = async function(){
    ...
    if(approvedArray && approvedArray.length > 0){
        ...
        for(let i = 0,len = approvedArray.length; i < len; i++){
          ...
          if(!isBNB){
            _self.approveTokens[i].isApproved = approvedArray[i].isApproved
            _self.approveTokens[i].gas = approvedArray[i].gas
          }
        }
        //更新oneTokens
        _self.ones.forEach(ele => {
          const whichCoins = _self.approveTokens.find(app => app.currency === ele.currency)
          const whichCoins2 = _self.approveTokens.find(app => app.currency === ele.currencyExtra)
          ele.isApproved = whichCoins?.isApproved
          ele.isApprovedExtra = true // 单币种不用再检测Libra是否授权
        })
        //更新twoTokens
          ...
          for(let i = 0; i < _self.approveTokens.length;i++){
            promiseAllarr[i] = new Promise((res,rej) => {
                Wallet.queryAllowance(_self.address,_self.approveTokens[i].currency,(pro)=>{
                    if(pro && pro > 0) {
                        res({isApproved:true,gas:+pro})
                    }else {
                        res({isApproved:false,gas:0})
                    }
                },(err) =>{res({isApproved:false,gas:0})})
            })
          }
          //全部请求
...
Farms.prototype.approveTokensAction = async function(type,item,callBack){
    ...
    if (type == 1){
      _self.approveInfo.isApproved1 = item.isApproved
      _self.approveInfo.isApproved2 = item.isApprovedExtra
    }
    ...
    if(!isApproved1 && isApproved2) {
      ...
      const approveCur1 = new Promise((resolve,reject) => {
          Wallet.approve(_self.currency1,_self.address,Math.pow(10,8),(res)=>{
              if(res){
                  notification.success({ message: error('A-Success'), description:_self.currency1 + ` ${error('A-A-Success')}`})
              }else {
                  notification.error({message: error('A-failed'),description:_self.currency1 + ` ${error('A-A-Failed')}`})
              }
              resolve(res)
          },(err) => {
              ...
              notification.error({
                  ...
                  description:_self.currency1 + `  ${error(msg)},${error('A-A-Failed')}`
              })
              reject(err)
          })
      })
      ...
      const approveCur2 = new Promise((resolve,reject) => {
          Wallet.approve(_self.currency2,_self.address,Math.pow(10,8),(res)=>{
              if(res){
                  notification.success({ message: error('A-Success'), description:_self.currency2 + ` ${error('A-A-Success')}`})
              }else {
                  notification.error({message: error('A-failed'),description:_self.currency2 + ` ${error('A-A-Failed')}`})
              }
              resolve(res)
          },(err) => {
              ...
              notification.error({
                  message: error('A-failed'),
                  description:_self.currency2 + `  ${error(msg)},${error('A-A-Failed')}`
              })
              reject(err)
          })
      })
      ...
      if(cur2_status){
        ...
        _self.approveInfo.isApprovedExtra = true
        _self.ones.forEach(ele => { ele.isApprovedExtra = true})
        _self.twos.forEach(ele => {
...
Farms.prototype.depositOneCurrency = function(){
  ...
  const {upperAddress,address,currency1,amount} = this
  console.log({upperAddress,address,currency1,amount})
  return new Promise((resolve,reject) => {
      Wallet.depositOne(upperAddress,address,currency1,amount,(res)=>{
          resolve(res)
        },(err)=>{
          console.log({err})
          reject(err)
...
Farms.prototype.configDepositInfo = function(type){
    ...
    let needData = type == 1 ? [
        ...
        new Promise((resolve) => {
              Wallet.balanceOf(_self.currency2,_self.address,(res)=>{
                  resolve(Number(res ? res : 0))
              },(err) =>{rej(err)})
        })
      ]
